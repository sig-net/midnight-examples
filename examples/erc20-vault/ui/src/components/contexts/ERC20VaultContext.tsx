...
function buildProviders(config: MidnightNodeConfig, wallet: Wallet): ERC20VaultProviders {
...
export function ERC20VaultContextProvider({
  children,
}: ERC20VaultContextProviderProps): JSX.Element {
  ...
  const identityQueryKey = useMemo(
    () => ["erc20vault-identity", config.networkId, wallet?.id ?? null, contractAddress, mpcPubkey],
  ...
  const contractQuery = useQuery<FoundContract<ERC20VaultContract>>({
    queryKey: ["erc20vault-contract", config.networkId, wallet?.id ?? null, contractAddress],
    enabled: providers !== null && contractAddress !== null && identity !== null,
    queryFn: async () => {
      if (!(providers && contractAddress)) {
        throw new Error(
          "erc20 vault not initialised: a wallet and a deployed network are required.",
        );
      }
  ...
  const readContractState = useCallback(async () => {
    if (!(providers && contractAddress)) {
      throw new Error(
        "erc20 vault not initialised: a wallet and a deployed network are required before reading state",
      );
    }
  ...
  return <ERC20VaultContext.Provider value={value}>{children}</ERC20VaultContext.Provider>;
}
