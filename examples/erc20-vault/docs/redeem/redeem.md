...
- **1.** startRedeem(...) burns the surrendered coin and records the request
  ...
  - Off chain, [`start-redeem.ts`](../../integration-tests/src/flows/start-redeem.ts)
    takes the vault account's next EVM nonce from its caller, funds the coin of the wrapper's
...
- **5.** completeRedeem(...) mints shielded(stataUnderlying) for the attested assets
  ...
  - The minted amount is the ATTESTED assets, deserialised from the output bytes
    ...
    are at least the underlying a supply of the same shares surrendered. Only the
...
The off-chain steps (2 to 4) each build a `SignetRequestResponseReader` over
...
[`runRedeemRoundTrip`](../../integration-tests/src/flows/redeem-round-trip.ts)
also needs the caller to already HOLD the shares it
