...
- **2.** supply(...) burns the surrendered coin and records the request
  ...
  - Off chain, [`supply.ts`](../../integration-tests/src/flows/supply.ts#L70)
...
- **5.** poll for the MPC's attestation
  ...
  - [`fetchSupplyOutcome`](../../integration-tests/src/flows/supply.ts#L160)
...
- **6.** completeSupply(...) mints shielded(stataToken) for the attested shares
  ...
  - The mint is supplier-only: the caller proves the secret behind the
    ...
    `supplyRefundCommitment` entry, and both the request record and the marker
...
- **6.** refund(...) re-mints when the supply never executed
  ...
  - For a supply the marker is `supplyRefundCommitment`. The commitment and the
    amount come from the settle view pinned at supply time, and the colour is
    the pinned `stataUnderlying` cell rather than a token address carried in the
    view, as a supply can only ever have burned that one colour. The taken arm's
    event map entry and marker are both consumed, and the value re-mints once
    after the join.
...
sequenceDiagram
    ...
    Note over User,Singleton: Step 2: supply(...) burns the surrendered coin and records the request
    User->>Vault: supply(...) surrendering a shielded underlying vault coin
    Vault->>Singleton: signBidirectional(...)
    ...
    alt the wrapper deposit executed (8-byte packed shares)
        ...
        Note over User,Vault: Step 6: refund(...) re-mints when the supply never executed
        User->>Vault: refund(...)
    end
```
