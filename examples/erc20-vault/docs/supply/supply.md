...
- **1.** approveStata(...) records the sign-only allowance request
  ...
  - The record is a plain 2-word request with the ERC20 `bool` schema, so it
    ...
    alongside the other approvals and the withdrawals, and the derivation path is the
...
- **2.** startSupply(...) burns the surrendered coin and records the request
  ...
  - Off chain, [`start-supply.ts`](../../integration-tests/src/flows/start-supply.ts)
    takes the vault account's next EVM nonce from its caller, funds the coin of the underlying's
...
- **5.** poll for the MPC's attestation
  ...
  - [`fetchExecutedSupplyOutput`](../../integration-tests/src/flows/complete-supply.ts)
    ...
    request's `outputDeserializationSchema` (the `uint256 shares` the wrapper
    returned) and re-packed per its `respondSerializationSchema` (the same
    shares as a `uint64`), which is the 8-byte output the settle circuit
  ...
  - The verified kind is also what routes step 6. Everything fetched here
...
- **6.** refundSupply(...) re-mints when the supply never executed
  ...
  - Each request kind has its own refund circuit (`refundWithdraw`,
    ...
    kind check, and `refundSupply` reads ONLY the supply settle-view map: a
...
with [`stataAvailable`](../../integration-tests/src/evm-stata.ts#L45) before any
spec runs and fails the run where it is absent, so
[`ensureStataApproved`](../../integration-tests/src/flows/approve-stata.ts#L113)
...
sequenceDiagram
    ...
    else the wrapper deposit never executed (failed or unviable, empty output)
