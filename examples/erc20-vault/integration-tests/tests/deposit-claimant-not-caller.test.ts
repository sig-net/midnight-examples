...
const recipientKeys = (): ShieldedTokenRecipient => {
  const { shieldedSecretKeys } = deriveAccountKeys(
    RECIPIENT_SEED,
...
const readRecipientVaultTokenBalance = async (): Promise<bigint> => {
  ...
  return withSyncedWalletFacade(keys, nodeConfig, (_facade, state) =>
    Promise.resolve(state.shielded.balances[color] ?? 0n),
  );
};

describe.skipIf(!process.env.RUN_INTEGRATION_TESTS)(
  "erc20-vault deposit → claim to a recipient that is not the caller e2e",
  () => {
    installFlowHooks();
    ...
    it(
      ...
      async () => {
        ...
        expect(recipientBalanceBefore).toBeGreaterThanOrEqual(0n);
      },
      15 * MINUTE,
    );
    ...
    it(
      ...
      async () => {
        const { requestId, claimed } = await runDepositRoundTrip(session, {
          ...
          reuseRequestId: env.DEPOSIT_CLAIMANT_NOT_CALLER_DEPOSIT_REQUEST_ID as
            RequestIdHex | undefined,
        ...
        banner([
          ...
          "",
          `  claim executed this run: ${String(claimed)}`,
          "",
