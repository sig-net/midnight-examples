...
export async function drainVaultErc20(
  ...
  // The token to drain; defaults to the suite's EVM_ERC20_CONTRACT_ADDRESS. The lending refund
  // passes Aave USDC so the vault holds none when the supply's transferFrom runs.
  const erc20Address = tokenAddress ?? requireEnv(env, "EVM_ERC20_CONTRACT_ADDRESS");
