...
/**
 * Poll until the redeem outcome resolves, then settle via completeRedeem / refund.
 *
 * @param context - The flow context.
...
export async function completeRedeem(
  ...
  while (
    Date.now() < end &&
    (outcome = await fetchRedeemOutcome(context, requestId)) === undefined
  ) {
    await new Promise((r) => setTimeout(r, 1000));
  }
  if (!outcome) throw new Error(`timed out waiting for a redeem attestation for ${requestId}`);
