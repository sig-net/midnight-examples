...
/**
 * Setup step: deal the example's EVM accounts their gas + tokens on the fork. User 1's deposit
 * account and own wallet account each get ETH + USDC (the deposit sources, on-chain and in the
 * UI), and the vault gets ETH (withdraw/approve/swap gas, deposits fund its USDC). Requires
 * EVM_RPC_URL to point at a Sepolia fork exposing anvil_* cheatcodes.
 *
 * @param env - The suite's env accumulator (reads EVM_RPC_URL, EVM_USER1_DEPOSIT_ADDRESS,
 *   EVM_USER1_WALLET_ADDRESS, EVM_VAULT_ACCOUNT_ADDRESS).
...
export async function dealForkEvmAccounts(env: NodeJS.ProcessEnv): Promise<void> {
  ...
  const userDeposit = requireEnv(env, "EVM_USER1_DEPOSIT_ADDRESS");
  ...
  try {
    await dealFork(provider, userDeposit, USER_USDC);
  ...
  console.log(
    `dealt on fork: user 1 deposit ${userDeposit} and wallet ${userWallet} <- 100 USDC + gas each; vault ${vault} <- gas`,
  );
}
