...
- **A signature poll timing out on a request the responder NEVER logged as
  ...
  event history has likely outgrown a single indexer page and the responder's
  ...
  past #100 is equally invisible to the tests' own polls. Confirm by counting
  ...
  the horizon). Recover by tearing the whole stack down and redeploying
  (`docker compose --profile fakenet down && docker compose up -d`, then the
  redeploy flow above) so the registry starts empty, and keep redeploy
  campaigns short of ~30 requests until the compose file pins a fakenet image
  whose SDK paginates the event read. Every fresh vault deploy leaves its
