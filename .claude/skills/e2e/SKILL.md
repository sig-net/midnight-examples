...
nine-spec suite (65 tests). The pipeline itself (globalSetup steps + flow test
...
echo 'SEPOLIA_FORK_RPC_URL=<a Sepolia RPC url>' >> .env   # REQUIRED: anvil forks Sepolia from it
...
a hard error, not an overwrite. On the Sepolia-forked anvil (chain id 11155111)
a fresh DEPLOY runs green in ONE pass: the derived accounts are dealt ETH +
real USDC via anvil cheatcodes, and the setup starts the fakenet responder
itself mid-run. The FLOW files are another matter: on a
16 GB Docker VM expect the proof-server OOM (see "Reading failures") to
interrupt the suite at some proving leg partway through the nine files:
...
- **`/e2e redeploy`**: a circuit changed (any `.compact` edit that alters a
  ...
  `EVM_VAULT_ACCOUNT_ADDRESS`, `EVM_USER1_DEPOSIT_ADDRESS` in `.env`, then
  rerun. The whole redeploy completes in ONE run: setup re-compiles (zk
  keygen, ~10 min, background the run), redeploys, re-derives, re-deals, and
  ...
  but on the local fork the new accounts are simply dealt funds by setup,
  nothing needs sweeping.)

## Ground rules (violating these wastes 10+ minutes per mistake)

- Run the suite from the repo root: `yarn test:erc20-vault:e2e` (all nine
...
- Expected per-spec test counts, in run order: `happy-day-e2e` **15**,
  ...
  **6**, `benchmark` **13**, `false-claimer` **6**, `bearer-transfer` **11**,
  `swap-e2e` **1**, `swap-refund-e2e` **1**, `deploy-only` **3** — 65 total
  (the two swap specs self-skip if Uniswap is absent, e.g. an un-forked
  anvil). `deploy-only` runs no deposit or withdrawal: selecting just that
  file (`yarn test:erc20-vault:e2e tests/deploy-only.test.ts`) runs the setup
...
- **Preflight `expected 0 to be greater than or equal to …`**: a derived EVM
  ...
  (`EVM_VAULT_ACCOUNT_ADDRESS`, `EVM_USER1_DEPOSIT_ADDRESS`) and rerun so
  setup re-derives and re-deals ETH + real USDC to the accounts on the fork.
