...
export function ensureWalletSeeds(env: NodeJS.ProcessEnv): void {
  ...
  if (Object.keys(generated).length > 0) {
    appendRepoDotEnv(
      generated,
      "test-harness setup: generated Midnight wallet seeds (root/deployer/user 1/mpc responder/user 2)",
    );
  }
}
 ...
 *
 * @param env - The suite's env accumulator, read for `FUND_WALLET_NIGHT_AMOUNT`.
 * @param rootNight - Root's current NIGHT balance, in base units.
 * @param unfundedCount - How many wallets still need funding.
 * @returns The NIGHT amount to send each unfunded wallet, in base units.
 * @throws {Error} If `FUND_WALLET_NIGHT_AMOUNT` is set to anything but a non-negative integer.
 */
function perChildAmount(env: NodeJS.ProcessEnv, rootNight: bigint, unfundedCount: number): bigint {
  ...
  if (override) {
    if (!/^\d+$/.test(override)) {
      throw new Error(
        `FUND_WALLET_NIGHT_AMOUNT must be a non-negative integer in NIGHT base units; got "${override}".`,
      );
    }
 ...
 * @throws {RootUnfundedError} When root holds too little to fund the children.
 */
