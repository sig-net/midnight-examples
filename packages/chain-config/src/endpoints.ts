...
/** The endpoint set of a {@link MidnightNodeConfig}, without the network id. */
export type Endpoints = Omit<MidnightNodeConfig, "networkId">;

/**
 * The local proof server endpoint. The proof server sees private witness
 * data, so it is always run locally rather than against a remote host.
 */
...
export const DEFAULT_ENDPOINTS: Record<NetworkId, Endpoints> = {
  ...
  // A consumer supplies them from its own environment: the Node reader
  // `getMidnightNodeConfig` in @midnight-examples/lib REQUIRES them for this
  // network, and names the variables to set.
  ["stagenet"]: {
    indexerUrl: "",
    indexerWsUrl: "",
    nodeUrl: "",
...
/**
 * Faucet URLs for the networks that publish one, for underfunded-wallet hints.
 * The local standalone chain funds via genesis, not a faucet, so it has no
 * entry. Stagenet's faucet URL is deliberately NOT published in this repo, and
 * the public *.midnight.network faucet URLs are omitted until confirmed: a
 * consumer supplies one from its own environment.
 */
 ...
 * to ws(s) and append the "/ws" path segment the indexer expects. Any consumer
 * that lets the indexer URL be overridden needs this, so it lives here rather
 * than being reimplemented per environment.
 *
